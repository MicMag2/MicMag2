...
class FSStrayField(Module):
    def __init__(self, method = "tensor"):
        super(FSStrayField, self).__init__()
        self.method = method
    ...
    def initialize(self, system):
        self.system = system
        self.calculator = FSStrayFieldCalculator(system.mesh, self.method, self.padding)
    ...
    def calculate(self, state, id):
        ...
        if id == "H_stray":
            ...
            print("cellvolume", self.system.mesh.cell_volume)
            E_stray = cache.E_stray = -MU0/2.0 * self.system.mesh.cell_volume * state.M.dotSum(state.H_stray)
            #print("HB E_stray:", E_stray)
            return E_stray

        else:
            raise KeyError("FSStrayField.calculate: %s" % id)

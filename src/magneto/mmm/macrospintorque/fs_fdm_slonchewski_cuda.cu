...
template <typename real>
void fs_fdm_slonchewski_cuda_impl(
...
#ifdef HAVE_CUDA_64
	if (cuda64)
	return fs_fdm_slonchewski_cuda_impl<double>(dim_x, dim_y, dim_z, delta_x, delta_y, delta_z, Ms, J, alpha, xi, alpha_hall, M, dM);
	else
#endif
	return fs_fdm_slonchewski_cuda_impl<float>(dim_x, dim_y, dim_z, delta_x, delta_y, delta_z, Ms, J, alpha, xi, alpha_hall, M, dM);
}

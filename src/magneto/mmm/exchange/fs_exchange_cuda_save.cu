	...
	if (sx < dim_x && sy < dim_y) {
		...
		if (Ms_i > 0) {
			...
			if (shared[3][TY][TX-1] != 0) {
				sum[0] += (shared[0][TY][TX-1]) * wx;
				sum[1] += (shared[1][TY][TX-1]) * wx;
				sum[2] += (shared[2][TY][TX-1]) * wx;
			}
			...
			if (shared[3][TY-1][TX] != 0) {
				sum[0] += (shared[0][TY-1][TX]) * wy;
				sum[1] += (shared[1][TY-1][TX]) * wy;
				sum[2] += (shared[2][TY-1][TX]) * wy;
			}
	...
	if (sx < dim_x && sy < dim_y && sz < dim_z) {
		...
		if (Ms_i > 0.0) {
			...
			if (shared[3][TZ][TY][TX-1]) {
				sum[0] += (shared[0][TZ][TY][TX-1]) * wx; 
				sum[1] += (shared[1][TZ][TY][TX-1]) * wx; 
				sum[2] += (shared[2][TZ][TY][TX-1]) * wx; 
			}
			...
			if (shared[3][TZ][TY-1][TX]) {
				sum[0] += (shared[0][TZ][TY-1][TX]) * wy; 
				sum[1] += (shared[1][TZ][TY-1][TX]) * wy; 
				sum[2] += (shared[2][TZ][TY-1][TX]) * wy; 
			}
			...
			if (shared[3][TZ-1][TY][TX]) {
				sum[0] += (shared[0][TZ-1][TY][TX]) * wz; 
				sum[1] += (shared[1][TZ-1][TY][TX]) * wz; 
				sum[2] += (shared[2][TZ-1][TY][TX]) * wz; 
			}
...
{
	{
		...
		const real wx = static_cast<real>(2.0/MU0) ;
		const real wy = static_cast<real>(2.0/MU0) ;
		const real wz = static_cast<real>(2.0/MU0) ;

	...
	if (sx < dim_x && sy < dim_y) {
		...
		if (Ms_i > 0) {
			...
			if (shared[3][TY][TX-1] != 0) {
				sum[0] -= (Dx_y[i]*shared[2][TY][TX-1] - Dx_z[i]*shared[1][TY][TX-1])/delta_x; 
				sum[1] -= (Dx_z[i]*shared[0][TY][TX-1] - Dx_x[i]*shared[2][TY][TX-1])/delta_x;  
				sum[2] -= (Dx_x[i]*shared[1][TY][TX-1] - Dx_y[i]*shared[0][TY][TX-1])/delta_x;  
			}
			...
			if (shared[3][TY-1][TX] != 0) {
				sum[0] -= (Dy_y[i]*shared[2][TY-1][TX] - Dy_z[i]*shared[1][TY-1][TX])/delta_y; 
				sum[1] -= (Dy_z[i]*shared[0][TY-1][TX] - Dy_x[i]*shared[2][TY-1][TX])/delta_y;  
				sum[2] -= (Dy_x[i]*shared[1][TY-1][TX] - Dy_y[i]*shared[0][TY-1][TX])/delta_y;
			}
	...
	if (sx < dim_x && sy < dim_y && sz < dim_z) {
		...
		if (Ms_i > 0.0) {
			...
			if (shared[3][TZ][TY][TX-1] != 0) {
				sum[0] -= (Dx_y[i]*shared[2][TZ][TY][TX-1] - Dx_z[i]*shared[1][TZ][TY][TX-1])/delta_x; 
				sum[1] -= (Dx_z[i]*shared[0][TZ][TY][TX-1] - Dx_x[i]*shared[2][TZ][TY][TX-1])/delta_x;  
				sum[2] -= (Dx_x[i]*shared[1][TZ][TY][TX-1] - Dx_y[i]*shared[0][TZ][TY][TX-1])/delta_x;  
			}
			...
			if (shared[3][TZ][TY-1][TX] != 0) {
				sum[0] -= (Dy_y[i]*shared[2][TZ][TY-1][TX] - Dy_z[i]*shared[1][TZ][TY-1][TX])/delta_y; 
				sum[1] -= (Dy_z[i]*shared[0][TZ][TY-1][TX] - Dy_x[i]*shared[2][TZ][TY-1][TX])/delta_y;  
				sum[2] -= (Dy_x[i]*shared[1][TZ][TY-1][TX] - Dy_y[i]*shared[0][TZ][TY-1][TX])/delta_y;
			}
			...
			if (shared[3][TZ-1][TY][TX] != 0) {
				sum[0] -= (Dz_y[i]*shared[2][TZ-1][TY][TX] - Dz_z[i]*shared[1][TZ-1][TY][TX])/delta_z; 
				sum[1] -= (Dz_z[i]*shared[0][TZ-1][TY][TX] - Dz_x[i]*shared[2][TZ-1][TY][TX])/delta_z;  
				sum[2] -= (Dz_x[i]*shared[1][TZ-1][TY][TX] - Dz_y[i]*shared[0][TZ-1][TY][TX])/delta_z;
			}
...
#ifdef HAVE_CUDA_64
	if (cuda64){
	//LOG_INFO << "run dmi on cuda 64";
        return dmi_cuda_impl<double>(dim_x, dim_y, dim_z, delta_x, delta_y, delta_z, periodic_x, periodic_y, periodic_z, Ms, Dx, Dy, Dz, M, H);
        }
	else
#endif
	{
        //LOG_INFO << "run dmi on cuda 32";
	return dmi_cuda_impl<float>(dim_x, dim_y, dim_z, delta_x, delta_y, delta_z, periodic_x, periodic_y, periodic_z, Ms, Dx, Dy, Dz, M, H);
	}
}

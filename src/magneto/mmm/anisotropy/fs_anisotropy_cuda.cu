...
__global__ void kernel_uniaxial_anisotropy(
	const real *Mx_ptr, const real *My_ptr, const real *Mz_ptr, const real *mu_ptr,
	...
	int N)
{
	...
	for (int i=tid; i<N; i+=threadN) {
		...
		const real Ms = Ms_ptr[i], k = k_ptr[i], mu=mu_ptr[i];
		if (Ms == 0.0 || k == 0.0) {
			Hx_ptr[i] = 0.0;
			Hy_ptr[i] = 0.0;
			Hz_ptr[i] = 0.0;
			...
			real f = 2.0 * k * d / (mu*real(MU0)); // / (real(MU0)*mu) ; //*Ms/mu
			Hx_ptr[i] = f * ax;
			Hy_ptr[i] = f * ay;
			Hz_ptr[i] = f * az;
			 E_ptr[i] = -k*d*d;
...
template <typename real>
double fs_uniaxial_anisotropy_cuda_impl(
	...
	VectorMatrix &H, const Matrix &Ms)
{
	...
	typename Matrix_const_cuda_accessor<real>::t Ms_acc(Ms), mu_acc(mu), k_acc(k);
...
template <typename real>
double fs_cubic_anisotropy_cuda_impl(
...
#ifdef HAVE_CUDA_64
	if (cuda64) 
	return fs_uniaxial_anisotropy_cuda_impl<double>(axis, k, mu, M, H, Ms); 
	else
#endif
	return fs_uniaxial_anisotropy_cuda_impl<float>(axis, k, mu, M, H, Ms);
}
...
#ifdef HAVE_CUDA_64
	if (cuda64)
	return fs_cubic_anisotropy_cuda_impl<double>(axis1, axis2, k, mu, M, H, Ms);
	else
#endif
	return fs_cubic_anisotropy_cuda_impl<float>(axis1, axis2, k, mu, M, H, Ms);
}

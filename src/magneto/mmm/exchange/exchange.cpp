	...
	VectorMatrix &H, double dx, double dy, double dz)
{
	...
	if (use_cuda) {
		...
		TIC("exchange");
		res = exchange_cpu(dim_x, dim_y, dim_z, delta_x, delta_y, delta_z, periodic_x, periodic_y, periodic_z, Ms, A, M, H, dx, dy, dz);
		TOC("exchange");
	}
	...
	VectorField &H, double dx, double dy, double dz)
{
	...
	return exchange(nx, ny, nz, dex, dey, dez, px, py, pz, Ms, A, M, H, dx, dy, dz);
}

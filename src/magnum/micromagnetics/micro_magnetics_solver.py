...
from magnum.mesh import Field
from .micro_magnetics import MicroMagnetics
from .io import writeOMF
#from .micro_magnetics import LandauLifshitzGilbert
...
class MicroMagneticsSolver(solver.Solver):
    ...
    def relaxFine(self, *args, **kwargs):
        # catch CVode, when using relax condition
        if self.evolver.__class__.__name__ == "Cvode":
          raise Exception("CVode is not usable to relax a system, yet. Please use rkf45.")

        return self.solve(solver.condition.fineRelaxed(*args, **kwargs))
    ...
    def minimize(self, max_dpns = 0.01, samples = 10, h_max = 1e-5, h_min = 1e-16):
        ...
        while len(dpnslist) < samples or max(dpnslist) > max_dpns:
            ...
            #plt.imshow(M_diff.to_numpy()[:,:,0,2])
            #plt.colorbar()
            #plt.show()
            #M_scale.assign(self.state.Ms)
            #M_scale.scale(1.0)
            #M_scale.scale(3DVector1.0)
            #M_diff.multiplyField(M_scale)

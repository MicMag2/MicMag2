	...
	for (int z=0; z<dim_z; ++z) {
		for (int y=0; y<dim_y; ++y) {	
			for (int x=0; x<dim_x; ++x) {
				...
				const Vector3d Dx_delta = Dx_acc.get(i)/delta_x;
				const Vector3d Dy_delta = Dy_acc.get(i)/delta_y;
				const Vector3d Dz_delta = Dz_acc.get(i)/delta_z;

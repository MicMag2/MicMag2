...
class HeisenbergModel(module.System):
    def __init__(self, world):
        super(HeisenbergModel, self).__init__(world.mesh)
#        self.mu = 2.16e-23 #TODO: Regolare queste variabili
        self.J = Field(world.mesh)
        self.J.fill(world.bodies[0].material.J)  #TODO: Set to 3.9e-21 (if that's correct)
        self.__state_class = None
        self.__world = world
        self.Ms = Field(world.mesh)
        self.Ms.fill(world.bodies[0].material.Ms)
    ...
    def __make_state_class(self):
        if not self.__state_class:
            ...
            class HeisenbergModelState(evolver.State):
                def __init__(this):
                    super(HeisenbergModelState, this).__init__(self.mesh)

                def finish_step(this):
                    this.M.normalize(this.system.mu)
            ...
            self.imbue_state_class_properties(HeisenbergModelState)
            self.__state_class = HeisenbergModelState

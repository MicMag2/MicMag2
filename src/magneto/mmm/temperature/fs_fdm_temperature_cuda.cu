...
#include "config.h"
#include "fs_fdm_temperature_cuda.h"
#include "curand.h"
...
__global__ 
void kernel_fs_fdm_temperature_2d(
	...
	const real seed)
{
	...
	if (x < dim_x && y < dim_y) {
		...
		if (Ms_k > 0.0 && (kelv_k > 0)) {
			real sigma = sqrt(2.*BOLTZMANN_CONSTANT*kelv_k*alpha_k/(GYROMAGNETIC_RATIO*Ms_k*dtime*MU0));
...
template <typename real>
__global__ 
void kernel_fs_fdm_temperature_3d(
...
template <typename real>
void fs_fdm_temperature_cuda_impl(
...
{
	{
		...
		if (is_2d) { // call 2d kernel
			...
			kernel_fs_fdm_temperature_2d<real><<<grid_dim, block_dim>>>(dim_x, dim_y, delta_x, delta_y, delta_z, Hth_acc.ptr_x(), Hth_acc.ptr_y(), Hth_acc.ptr_z(), Ms_acc.ptr(), alpha_acc.ptr(), kelv_acc.ptr(), dtime, step, seed);
			...
			kernel_fs_fdm_temperature_3d<real><<<grid_dim, block_dim>>>(dim_x, dim_y, dim_z, delta_x, delta_y, delta_z, Hth_acc.ptr_x(), Hth_acc.ptr_y(), Hth_acc.ptr_z(), Ms_acc.ptr(), alpha_acc.ptr(), kelv_acc.ptr(), dtime, step, seed);
...
#ifdef HAVE_CUDA_64
	if (cuda64)
	return fs_fdm_temperature_cuda_impl<double>(dim_x, dim_y, dim_z, delta_x, delta_y, delta_z, Ms, alpha, kelv, dtime, step, seed, Hth);
	else
#endif
	return fs_fdm_temperature_cuda_impl<float>(dim_x, dim_y, dim_z, delta_x, delta_y, delta_z, Ms, alpha, kelv, dtime, step, seed, Hth);
}

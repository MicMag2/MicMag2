...
if [[ `basename $thisDir` != "src" ]]; then
	...
	if nvcc --version 2>/dev/null; then
		...
		make $@
		#cmake .. -DPYTHON_LIBRARY=$(which python3) -DCMAKE_CXX_FLAGS=-isystem\ ${CONDA_PREFIX}/include
		#cmake .. && make $@
		...
		echo "trying GPU compiling..." && rm CMakeCache.txt && cmake .. -DPYTHON_LIBRARY=$(which python3) -DPYTHON_INCLUDE_DIR=${CONDA_PREFIX}/include/python3.8 -DENABLE_CUDA_64=on
		make $@ 

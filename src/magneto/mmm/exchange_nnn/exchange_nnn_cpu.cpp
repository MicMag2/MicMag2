/*
 * Copyright 2012, 2013, 2023 by the Micromagnum and MicMag2 authors.
 *
...
static double exchange_nnn_cpu_nonperiodic(
...
static double exchange_nnn_cpu_periodic(
	int dim_x, int dim_y, int dim_z,
	double delta_x, double delta_y, double delta_z,
	...
	VectorMatrix &H, double dx, double dy, double dz)
{
	...
	if (periodic) {
		return exchange_nnn_cpu_periodic(dim_x, dim_y, dim_z, delta_x, delta_y, delta_z, periodic_x, periodic_y, periodic_z, Ms, A, M, H);
	} else {
		return exchange_nnn_cpu_nonperiodic(dim_x, dim_y, dim_z, delta_x, delta_y, delta_z, Ms, A, M, H, dx, dy, dz);
	}
}
		...
		for (int y=0; y<dim_y; ++y) {	
			for (int x=0; x<dim_x; ++x) {
				...
				if (x >       0) {
					...
					if (Ms_l != 0.0) sum += ((M_acc.get(idx_l) / Ms_l) - M_i*dx) * wx;
				}
				if (x < dim_x-1) {
					const double Ms_r = Ms_acc.at(idx_r);	
					if (Ms_r != 0.0) sum += ((M_acc.get(idx_r) / Ms_r) - M_i*dx) * wx;
				}
				// up / down (Y)
				if (y >       0) {
					const double Ms_u = Ms_acc.at(idx_u);
					if (Ms_u != 0.0) sum += ((M_acc.get(idx_u) / Ms_u) - M_i*dy) * wy;
				}
				if (y < dim_y-1) {
					const double Ms_d = Ms_acc.at(idx_d);
					if (Ms_d != 0.0) sum += ((M_acc.get(idx_d) / Ms_d) - M_i*dy) * wy;
				}
				// forward / backward (Z)
				if (z >       0) {
					const double Ms_f = Ms_acc.at(idx_f);
					if (Ms_f != 0.0) sum += ((M_acc.get(idx_f) / Ms_f) - M_i*dz) * wz;
				}
				if (z < dim_z-1) {
					const double Ms_b = Ms_acc.at(idx_b);
					if (Ms_b != 0.0) sum += ((M_acc.get(idx_b) / Ms_b) - M_i*dz) * wz;
				}
...
{
	const int dim_xy = dim_x * dim_y;
	const double wx = 1.0 / (12.0 * delta_x * delta_x);
		...
		for (int y=0; y<dim_y; ++y) {	
			for (int x=0; x<dim_x; ++x) {
				...
				if (periodic_x) {
					...
					if (x == dim_x-1) idx_rr -= dim_x;
					if (x == dim_x-2) idx_rr -= dim_x;
				}
				if (periodic_y) {
					if (y ==       0) idx_u += dim_xy;
				...
				// left / right (X)
				const double Ms_l = Ms_acc.at(idx_l);
				const double Ms_ll = Ms_acc.at(idx_ll);
				const double Ms_r = Ms_acc.at(idx_r);
				const double Ms_rr = Ms_acc.at(idx_rr);
				sum+= ( -1.0* M_acc.get(idx_ll)/Ms_ll	+16.0*M_acc.get(idx_l)/Ms_l - 30.0*M_acc.get(i)/Ms + 16.0*M_acc.get(idx_r)/Ms_r   -1.0*M_acc.get(idx_rr)/Ms_rr ) * wx;

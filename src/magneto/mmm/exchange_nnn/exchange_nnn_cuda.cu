...
double exchange_nnn_cuda(
	...
	bool cuda64)
{
        //LOG_INFO << "run exchange on gpu";
#ifdef HAVE_CUDA_64
	if (cuda64)
	return exchange_nnn_cuda_impl<double>(dim_x, dim_y, dim_z, delta_x, delta_y, delta_z, periodic_x, periodic_y, periodic_z, Ms, A, M, H);
	else
#endif
	return exchange_nnn_cuda_impl<float>(dim_x, dim_y, dim_z, delta_x, delta_y, delta_z, periodic_x, periodic_y, periodic_z, Ms, A, M, H);
}

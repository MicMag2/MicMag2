...
class FSAnisotropyField(module.Module):
    def __init__(self):
        super(FSAnisotropyField, self).__init__()

    def calculates(self):
        return ["H_aniso", "E_aniso"]

    def params(self):
        return ["k_uniaxial", "k_cubic", "axis1", "axis2", "mu", "Ms"]
    ...
    def initialize(self, system):
        ...
        self.Ms = Field(self.system.mesh); self.Ms.fill(0.0)
    ...
    def on_param_update(self, id):
        if id in self.params():
            ...
            def compute_uniaxial(state, H_aniso):
                #print '##################################################################################' +  str(mu.get(0,0,0))
                tmpfield = magneto.fs_uniaxial_anisotropy(axis1, k_uni, mu, state.M, H_aniso, Ms)
                #print("aniso, ", tmpfield.get(400,400,0))
                return magneto.fs_uniaxial_anisotropy(axis1, k_uni, mu, state.M, H_aniso, Ms)

            def compute_cubic(state, H_aniso):
                return magneto.fs_cubic_anisotropy(axis1, axis2, k_cub, mu, state.M, H_aniso)

            def compute_uniaxial_and_cubic(state, H_aniso):
                tmp = VectorField(self.system.mesh)
                E0 = magneto.fs_uniaxial_anisotropy(axis1, k_uni, mu, state.M, tmp,Ms)
                #print("HB E_ani: ", E0)
                E1 = magneto.fs_cubic_anisotropy(axis1, axis2, k_cub, mu, state.M, H_aniso)

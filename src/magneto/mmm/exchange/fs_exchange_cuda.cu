	...
	if (sx < dim_x && sy < dim_y) {
		...
		if (Ms_i > 0) {
			...
			if (shared[3][TY][TX-1] != 0) {
				sum[0] += (shared[0][TY][TX-1]);
				sum[1] += (shared[1][TY][TX-1]);
				sum[2] += (shared[2][TY][TX-1]);
			}
			...
			if (shared[3][TY-1][TX] != 0) {
				sum[0] += (shared[0][TY-1][TX]);
				sum[1] += (shared[1][TY-1][TX]);
				sum[2] += (shared[2][TY-1][TX]);
			}
	...
	if (sx < dim_x && sy < dim_y && sz < dim_z) {
		...
		if (Ms_i > 0.0) {
			...
			if (shared[3][TZ][TY][TX-1]) {
				sum[0] += (shared[0][TZ][TY][TX-1]); 
				sum[1] += (shared[1][TZ][TY][TX-1]); 
				sum[2] += (shared[2][TZ][TY][TX-1]); 
			}
			...
			if (shared[3][TZ][TY-1][TX]) {
				sum[0] += (shared[0][TZ][TY-1][TX]); 
				sum[1] += (shared[1][TZ][TY-1][TX]); 
				sum[2] += (shared[2][TZ][TY-1][TX]); 
			}
			...
			if (shared[3][TZ-1][TY][TX]) {
				sum[0] += (shared[0][TZ-1][TY][TX]); 
				sum[1] += (shared[1][TZ-1][TY][TX]); 
				sum[2] += (shared[2][TZ-1][TY][TX]); 
			}
...
{
	{
		...
		if (is_2d) { // call 2d kernel
			...
			#define EXCH_2D(bx,by) if (periodic_x == bx && periodic_y == by) kernel_exchange_2d<real, bx, by><<<grid_dim, block_dim>>>(Mx, My, Mz, Hx, Hy, Hz, Ms_acc.ptr(), A_acc.ptr(), dim_x, dim_y);
			EXCH_2D(false, false)
			EXCH_2D(false,  true)
			EXCH_2D( true, false)
			EXCH_2D( true,  true)
			...
			#define EXCH_3D(bx,by,bz) if (periodic_x == bx && periodic_y == by && periodic_z == bz) kernel_exchange_3d<real, bx, by, bz><<<grid_dim, block_dim>>>(Mx, My, Mz, Hx, Hy, Hz, Ms_acc.ptr(), A_acc.ptr(), dim_x, dim_y, dim_z, logical_grid_dim_y);
...
#ifdef HAVE_CUDA_64
	if (cuda64)
	return fs_exchange_cuda_impl<double>(dim_x, dim_y, dim_z, delta_x, delta_y, delta_z, periodic_x, periodic_y, periodic_z, Ms, A, M, H);
	else
#endif
	return fs_exchange_cuda_impl<float>(dim_x, dim_y, dim_z, delta_x, delta_y, delta_z, periodic_x, periodic_y, periodic_z, Ms, A, M, H);
}

	...
	VectorMatrix &dM)
{
	...
	for (int n=0; n<N; ++n) {
		...
		//std::cout << xi_n << "    " << alpha_hall_n << std::endl;
		...
		dM_n.x += gamma_pr * a_j * ((1. + xi_n*alpha_n) * MxMxp.x/Ms_n + (xi_n - alpha_n) * Mxp.x);
		dM_n.y += gamma_pr * a_j * ((1. + xi_n*alpha_n) * MxMxp.y/Ms_n + (xi_n - alpha_n) * Mxp.y);
		dM_n.z += gamma_pr * a_j * ((1. + xi_n*alpha_n) * MxMxp.z/Ms_n + (xi_n - alpha_n) * Mxp.z);
		dM_acc.set(n, dM_n);
